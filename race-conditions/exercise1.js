...
function incrementCounterJere() {
    return new Promise((resolve) => {
        setTimeout(() => {
            const currentValueJere = balanceParaleloJere;
            setTimeout(() => {
                balanceParaleloJere = currentValueJere + 1;
                resolve();
            }, 200);
        }, Math.random() * 200);
    });
...
async function contadorBalanceJere(numberJere) {
    for (let i = 0; i < numberJere; i++) {
        await incrementCounterJere();
    }
    console.log(`Listo: ${balanceParaleloJere}`);
}

contadorBalanceJere(4);
